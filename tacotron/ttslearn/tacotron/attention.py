...
class BahdanauAttention(nn.Module):
    ...
    def __init__(self, encoder_dim=128, decoder_dim=1024, hidden_dim=128):
        super().__init__()
...
class LocationSensitiveAttention(nn.Module):
    ...
    def __init__(
        ...
        super().__init__()
        self.mlp_enc = nn.Linear(encoder_dim, hidden_dim)        
        self.mlp_enc2 = nn.Linear(encoder_dim, hidden_dim) #追加
        self.mlp_dec = nn.Linear(decoder_dim, hidden_dim, bias=False)
    ...
    def forward(
        ...
        if self.processed_memory2 is None:
            self.processed_memory2 = self.mlp_enc2(energy_outs)
        ...
        # エネルギーのアテンション重みを一様分布で初期化
        if att_prev2 is None:
            att_prev2 = 1.0 - make_pad_mask(energy_lens).to(
                device=decoder_state.device, dtype=decoder_state.dtype
            )
            att_prev2 = att_prev2 / energy_lens.unsqueeze(-1).to(energy_outs.device)
