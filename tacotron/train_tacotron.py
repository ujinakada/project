...
@torch.no_grad()
def eval_model(
    step, model, writer, in_feats, in_lens, out_feats, out_lens, is_inference, energy_feats, energy_lens, pitch_feats, pitch_lens, ex_feats, ex_lens, max_lens
):
    # 最大3つまで
    N = min(len(in_feats), 3)

    if is_inference:
        outs, outs_fine, att_ws, att_w2s, out_lens = [], [], [], [], []
        for idx in range(N):
            out, out_fine, _, att_w, att_w2 = model.inference(in_feats[idx][: max_lens[idx]], energy_feats[idx][: max_lens[idx]], pitch_feats[idx][: max_lens[idx]])
            outs.append(out)
        ...
        outs, outs_fine, _, att_ws, att_w2s = model(in_feats, in_lens, energy_feats, energy_lens, pitch_feats, pitch_lens, max_lens, out_feats,)
    ...
    for idx in range(N):
        ...
        plt.close()
        att_2w = att_w2s[idx][: out_lens[idx] // rf, : in_lens[idx]]#追加
        fig = plot_attention(att_2w)#
        writer.add_figure(f"{group}/attention_ex", fig, step)#
        plt.close()#
...
def train_step(
    ...
    # Run forwaard
    outs, outs_fine, logits, _, _2 = model(in_feats, in_lens, energy_feats, energy_lens, pitch_feats, pitch_lens, max_lens, out_feats)
...
def train_loop(config, device, model, optimizer, lr_scheduler, data_loaders, writer):
    ...
    for epoch in get_epochs_with_optional_tqdm(config.tqdm, nepochs):
        for phase in data_loaders.keys():
            ...
            running_losses = {}
            for idx, (in_feats, in_lens, out_feats, out_lens, stop_flags, energy_feats, energy_lens, pitch_feats, pitch_lens, ex_feats, ex_lens, max_lens) in tqdm(
                enumerate(data_loaders[phase]), desc=f"{phase} iter", leave=False
            ):
                # ミニバッチのソート
                in_lens, indices = torch.sort(in_lens, dim=0, descending=True)
                in_feats, out_feats, out_lens, energy_feats, energy_lens, pitch_feats, pitch_lens, ex_feats, ex_lens, max_lens = (
                    in_feats[indices].to(device),
                    out_feats[indices].to(device),
                    out_lens[indices].to(device),
                    energy_feats[indices].to(device),
                    energy_lens[indices].to(device),
                    pitch_feats[indices].to(device),
                    pitch_lens[indices].to(device),
                    ex_feats[indices].to(device),
                    ex_lens[indices].to(device),
                    max_lens[indices].to(device),
                )
...
@hydra.main(config_path="conf/train_tacotron", config_name="config", version_base=None)
